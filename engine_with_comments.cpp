...
//long doubles zijn geweldig, snel en superieur
//functies spreken wel een beetje voor zich, denk ik
void Mandelbrot(long double range[4], int iterations, long double xres, long double yres)
{
...
int main()
{
	...
	PLOT.open("TEXT/plot.txt", std::ofstream::out | std::ofstream::trunc); //open en *wis* plot
	R9K.open("TEXT/R9K.txt");
	CHECK.open("TEXT/check.txt", std::ofstream::out | std::ofstream::trunc); //open en *wis* check
	
	int precision = std::numeric_limits<double>::max_digits10;
	PLOT << std::setprecision(precision); //zorgt ervoor dat long doubles met precisie naar de tekstbestanden worden geschreven
	
	long double range[4];
	int screen[2], iterations, fractal;
	R9K >> fractal >> range[0] >> range[1] >> range[2] >> range[3] >> screen[0] >> screen[1];
	switch(fractal) //een soort lange if elif blok, code spreekt verder wel een beetje voor zich
	{
		case 0:
		{
				INPUT >> iterations;
		...
		case 1:
		{
				...
				for(int i=0; i < n; i++) //bereken de coördinaten voor de hoeken van het sierpinski fractaal
				{
		...
		case 3:
		{
				int r;
				INPUT >> iterations >> r;
				switch(r-1) //parameterveld van IFS heeft verschillende versies met verschillende arrays van verschillende groottes
				{
					case 0:
					{
	...
	CHECK << '1'; //zeg tegen gui.py dat engine klaar is
	CHECK.close();
	exit(EXIT_FAILURE); //sluit engine.exe
}